...
export interface OverlayState { reads: CardRead[]; bestId: number | null; frameW: number; frameH: number }
...
export function drawReads(ctx: CanvasRenderingContext2D, reads: CardRead[], bestId: number | null, scaleX: number, scaleY: number) {
  for (const read of reads) {
    ...
    if (isBest) {
      ctx.fillStyle = '#39ff6a'; ctx.font = 'bold 13px sans-serif';
