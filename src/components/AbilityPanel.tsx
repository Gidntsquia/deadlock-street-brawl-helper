 ...
 *  `later`: not yet spent. An ability whose unlock is this round gets an UNLOCK badge and a lilac ring; a locked one
 *  is dimmed. Sized by the CSS variable `--u` (one reference px of the 485-px-wide in-game panel). */
    ...
    >
      {panel.slots.map((s) => (
        <div key={s.key} className="ap-col" data-ability={s.name}>
          ...
          <div className={`ap-ability ap-unlock-${s.unlock}`} data-unlock={s.unlock}>
            {s.unlock === 'now' && <div className="ap-badge">UNLOCK</div>}
