 ...
 */
export function choosePopulation(analytics: HeroAnalytics): {
  items: AnalyticsPopulation;
  abilities: AnalyticsPopulation;
  info: BuildPopulation;
} {
  ...
  return {
    ...
    info: {
      kind: useTop ? 'top' : 'all',
      minBadge: useTop ? top!.min_average_badge : null,
...
export function stylePopulations(input: GeneratorInput): Population[] {
  ...
  const usage = styles.map((s) => {
    const n = Math.max(1, ...s.item_stats.map((x) => x.matches));
    return new Map(s.item_stats.map((x) => [x.item_id, x.matches / n]));
  });
  const named = styles.map((s, k) => {
    ...
    const defining = [...usage[k]]
      ...
      .map(([id, u]) => ({
        item: catalog.get(id)!,
        edge: u - Math.max(0, ...usage.filter((_, j) => j !== k).map((o) => o.get(id) ?? 0)),
      }))
      .filter((d) => d.edge > 0.15)
      .sort((a, b) => b.edge - a.edge || a.item.id - b.item.id)
      .slice(0, 5);
  ...
  return named.map(({ s, defining, name }): Population => {
    ...
    return {
      items: {
        item_stats: s.item_stats,
        ability_order_stats: s.ability_order_stats,
        permutation_stats: base.items.permutation_stats,
      },
      abilities: {
        item_stats: abilities.item_stats,
        ability_order_stats: abilities.ability_order_stats,
        permutation_stats: base.items.permutation_stats,
      },
      info: {
        kind: 'top',
        minBadge: base.info.minBadge,
        matches: s.matches,
        abilitySequenceKind:
          seqMatches >= MIN_TOP_SEQ_MATCHES || base.info.abilitySequenceKind === 'top' ? 'top' : 'all',
        style: { key: s.key, share: s.share, seed, anchors, exclude, defining, name: finalName, tagline },
      },
    };
  });
}
...
export function generateBuild(input: GeneratorInput, arch: Archetype, population?: Population): Build {
  ...
  const {
    weights: WEIGHTS,
    winShrinkFrac: WIN_SHRINK_FRAC,
    minUsage: MIN_USAGE,
    maxItems: MAX_ITEMS,
    minItems: MIN_ITEMS,
    maxUpgradeSteps: MAX_UPGRADE_STEPS,
    slotCap: SLOT_CAP,
    maxActives: MAX_ACTIVES,
    phaseTimeS: PHASE_TIME_S,
    tierMin: TIER_MIN,
    pairMinMatches,
  } = PARAMS;
  ...
  const effMed = median(rawEff),
    kitMed = median(rawKit);
  for (const p of pre) {
    if (p.eff === 0) p.eff = effMed;
    if (p.kit === 0) p.kit = kitMed;
  }
  const effMax = Math.max(1e-9, ...pre.map((p) => p.eff)),
    kitMax = Math.max(1e-9, ...pre.map((p) => p.kit));
  const scored: Scored[] = pre.map(({ item, stat, eff, kit: k }) => {
    ...
    const base =
      WEIGHTS.popularity * Math.sqrt(pop) +
      WEIGHTS.winLift * winLift +
      WEIGHTS.efficiency * (eff / effMax) +
      WEIGHTS.kit * (k / kitMax) +
    ...
    return {
      ...
      eff: eff / effMax,
      kit: k / kitMax,
      base: base * arch.slotBias[item.item_slot_type],
    };
  });
  ...
  const chainPair = (it: Item): [string, string] | null => {
    ...
    const upgradeOfIt = chosen.find(
      (c) =>
        c.s.item.component_items.includes(it.class_name) &&
        !consumed.has(it.class_name) &&
        ![...consumed.values()].includes(c.s.item.class_name),
    );
  ...
  const tierShortfall = () =>
    Object.entries(TIER_MIN)
      .filter(([t, min]) => (tierCount[+t] ?? 0) < min)
      .map(([t]) => +t);
  ...
  while (finals < MAX_ITEMS && chosen.length < MAX_ITEMS + MAX_UPGRADE_STEPS) {
    ...
    let best: { s: Scored; score: number; reasons: string[]; inChain: boolean; chain: [string, string] | null } | null =
      null;
    for (const s of scored) {
      ...
      for (const c of chosen) {
        const l = pair.get(`${it.id}:${c.s.item.id}`);
        if (l !== undefined) {
          syn += l;
          n++;
        }
      ...
      if (!best || score > best.score || (score === best.score && it.id < best.s.item.id)) {
        ...
        if (s.pop > 0.5)
          reasons.push(
            `bought in ${((s.stat.matches / maxMatches) * 100).toFixed(0)}% of ${hero.name} games (relative)`,
          );
    ...
    if (best.chain) consumed.set(best.chain[0], best.chain[1]);
    else {
  ...
  chosen.sort(
    (a, b) =>
      a.s.stat.avg_buy_time_s - b.s.stat.avg_buy_time_s || a.s.item.cost - b.s.item.cost || a.s.item.id - b.s.item.id,
  );
  for (let i = 0; i < chosen.length; i++) {
    ...
    if (j > i) {
      const [comp] = chosen.splice(j, 1);
      chosen.splice(i, 0, comp);
    }
  }
  let running = 0;
  ...
  const buildItems: BuildItem[] = chosen.map((c, i) => {
    ...
    const reasons = upgradesFrom
      ? [`upgrades ${upgradesFrom.name} already in the build; pays only the ${paidCost} soul difference`, ...c.reasons]
      : c.reasons;
    return {
      item: c.s.item,
      phase,
      ...
      reasons,
      usageRate: c.s.pop,
      winRate: c.s.stat.wins / c.s.stat.matches,
      avgBuyTimeS: t,
    };
  });
  ...
  if (phases.size < 3)
    buildItems.forEach((b, i) => {
      b.phase = i < buildItems.length / 3 ? 'early' : i < (2 * buildItems.length) / 3 ? 'mid' : 'late';
    });

  const ab = pickAbilityOrder(hero, abilities, pop.abilities.ability_order_stats);
  const style = pop.info.style;
  return {
    key: style ? style.key : arch.key,
    name: style ? style.name : arch.name,
    tagline: style ? style.tagline : arch.tagline,
    heroId: hero.id,
    items: buildItems,
    totalCost: running,
    abilityOrder: ab.steps,
    abilityOrderSupport: ab.support,
    population: pop.info,
  };
}
